...
describe('CampaignsController', () => {
  ...
  describe('agnostic campaigns', () => {
    ...
    before(() =>
      createCampaign({ collectiveId: Collective.invisibleId }).then(
        campaign => {
    ...
    it('should be viewable one-by-one', () =>
      agent
        .get(`${url}${urls.Campaigns.show.url(campaignId)}`)
        .set('Accept', 'text/html')
        .then(res => expect(res.status).to.equal(200)));

    it('should display an index of all of them', () =>
      agent
        .get(`${url}${urls.Campaigns.url()}`)
        .then(res =>
          expect(res.text).to.have.string(
            'This is a list of all the campaigns!!!',
          ),
        ));
    ...
    describe('Users', () => {
      ...
      beforeEach(() =>
        createUser({ role: 'User' })
          ...
          .then(() =>
            agent
              .post(`${url}${urls.Campaigns.join.url(campaignId)}`)
              .send({ _csrf })
              .then(res => expect(res.status).to.equal(200)),
          ),
      );
      ...
      it('should be able to participate in an agnostic campaign', () =>
        agent
          .post(`${url}${urls.CreatePost.url(campaignId)}`)
