...
export default function (disputeId) {
  return new Vue({
    ...
    methods: {
      save() {
        return updateAdmins(this.disputeId, this.assigned.map(a => a.id))
          .then(() => new RestAlert({
      ...
      setDisputeId(nextDisputeId) {
        ...
        getAvailableAndAssignedAdmins(this.disputeId)
    ...
    render(h) { // eslint-disable-line
      return (
        <div class="pb3">
          ...
          />
          <button class='-fw -k-btn btn-primary -fw-700 mt2 mb2'
            on-click={() => this.save()}
            type="button">
            Save
          </button>
