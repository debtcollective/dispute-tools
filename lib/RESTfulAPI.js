  ...
  prototype: {
    ...
    init(config) {
      ...
      if (!this.paginate.pageSize) {
        throw new Error(
          'Must provide a pageSize for pagination {pageSize : 50 }',
        );
      }
    ...
    _filtersToQueryBuilder(filters, parentKey) {
      ...
      Object.keys(filters).forEach(key => {
        ...
        if (
          restApi.filters.allowedFields.includes(column) &&
          value &&
          value !== ''
        ) {
    ...
    _buildPagination() {
      ...
      const orderClause = restApi.queryBuilder._queryMethodCalls.splice(
        orderIndex,
        1,
      )[0];
      ...
      return restApi.queryBuilder.count('*').then(result => {
        const total = result[0].count;
        const pages = Math.ceil(total / restApi.paginate.pageSize);
        ...
        restApi.queryBuilder._queryMethodCalls.splice(countIndex, 1);
        ...
        return restApi.queryBuilder.page(
          currentPage,
          restApi.paginate.pageSize,
        );
      });
    },

    build() {
      this._buildOrder()._buildFilters();

      return this._buildPagination().then(queryBuilder => queryBuilder);
    },
  },
});
