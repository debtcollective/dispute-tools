    ...
    _buildPagination() {
      ...
      restApi.queryBuilder._queryMethodCalls.forEach((item) => {
        if (item.method === 'orderBy') {
          const index = restApi.queryBuilder._queryMethodCalls.indexOf(item);
        ...
        .then((result) => {
          ...
          restApi.queryBuilder._queryMethodCalls.forEach((item) => {
            if (item.method === 'count') {
              const index = restApi.queryBuilder._queryMethodCalls.indexOf(item);
              restApi.queryBuilder._queryMethodCalls = restApi.queryBuilder
                ._queryMethodCalls.splice(index, 1)[0];
            }
          });
